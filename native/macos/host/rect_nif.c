// rect_nif — in-process render backend NIF (macOS host).
//
...
// Rect.Bridge — same protocol (JSON frames + event tags), no wire.
//
// For the prototype, render() logs frames (headless-verifiable) and calls the
// weak `rect_ui_render` hook if the host provides one (GUI mode). Event
...
__attribute__((weak)) void rect_ui_render(const char *window, const char *json);
...
static ERL_NIF_TERM render_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    fprintf(stderr, "[rect_nif] render window=%s json=%s\n", wbuf, jbuf);
    fflush(stderr);

    if (rect_ui_render)
        rect_ui_render(wbuf, jbuf);
...
static ERL_NIF_TERM register_window_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    g_have_pid = 1;
    fprintf(stderr, "[rect_nif] window process registered\n");
    fflush(stderr);
    return enif_make_atom(env, "ok");
}
...
//   {rect_ui_event, <<Event>>, #{<<"tag">> => <<Tag>>}}
void rect_ui_emit_event(const char *event, const char *tag) {
    if (!g_have_pid)
        return;
    ...
    ERL_NIF_TERM msg = enif_make_tuple3(env, enif_make_atom(env, "rect_ui_event"), ev_bin, params);
...
ERL_NIF_INIT(rect_nif, funcs, NULL, NULL, NULL, NULL)
