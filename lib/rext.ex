defmodule Rext do
  @moduledoc """
  Rext — a BEAM-on-desktop UI framework for Elixir.

  Rext is mob's desktop sibling. It keeps mob's programming model verbatim — a
  ...
  the same channel — but it commits to *desktop* paradigm and concerns:
    ...
    * the unit is `Rext.Window`, and apps run many windows at once;
    * the render backend is SwiftUI on macOS and Compose Multiplatform on
      ...
      desktop targets);
    * dev/agent tooling lives in `rext_dev`, project generation in `rext_new`.

  Rext was seeded by copying mob's platform-agnostic core rather than sharing a
  library up front; a common `beam_native_ui` package is a later refactor if the
  duplication proves significant and enduring.

  ## Opening a window

      {:ok, _pid} = Rext.open(MyApp.CounterWindow, id: "main", title: "Counter")
  """

  @doc """
  Open a window under the window supervisor.

  Options are passed to `Rext.Window.start_link/3` (`:id`, `:title`, `:size`).
  """
  @spec open(module(), keyword()) :: DynamicSupervisor.on_start_child()
  def open(window_module, opts \\ []) do
    DynamicSupervisor.start_child(
      Rext.WindowSupervisor,
      %{
        id: {Rext.Window, opts[:id] || "main"},
        start: {Rext.Window, :start_link, [window_module, %{}, opts]},
        restart: :transient
      }
    )
  end

  @doc "Boot every window declared by an app module (see `Rext.App`)."
  @spec boot(module()) :: :ok
