...
| **One renderer draws one window** | 🟡 | 🟡 | 🟡 | 🟡 | **The blocking architectural gap.** A multi-window app currently needs one renderer process per window; `mix rext.run` launches only the primary. Multi-window *logic* works over dist today — the rendering doesn't |
